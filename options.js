...
function restoreOptions() {
  initVariables();
  var reqInterval = localStorage["DT.RequestInterval"];
...
function saveOptions() {
  ...
  localStorage["DT.RequestInterval"] = interval;

  for (var i=0; i<radioNotifications.length; i++) {
    if (radioNotifications[i].checked) {
      localStorage["DT.Notifications"] = radioNotifications[i].value;
      break;
    }
  }
  ...
  for (var i=0; i<radioBackgroundTabs.length; i++) {
    if (radioBackgroundTabs[i].checked) {
      localStorage["DT.BackgroundTabs"] = radioBackgroundTabs[i].value;
      break;
    }
  }
}
